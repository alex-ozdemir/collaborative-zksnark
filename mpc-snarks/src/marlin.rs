...
use super::{MpcPairingEngine, MpcVal};
use super::poly::pc::MpcMarlinKZG10;
use crate::mpc::MpcWire;
...
fn prover_message_publicize(p: ahp::prover::ProverMsg<MpcVal<ark_bls12_377::Fr>>) -> ahp::prover::ProverMsg<ark_bls12_377::Fr> {
    use ahp::prover::ProverMsg;
    match p {
...
pub fn pf_publicize(k: Proof<MpcVal<ark_bls12_377::Fr>, MpcMarlinKZG10>) -> Proof<ark_bls12_377::Fr, LocalMarlinKZG10> {
    ...
    let r = Proof::<ark_bls12_377::Fr, LocalMarlinKZG10> {
        ...
        pc_proof: super::poly::pc::lower_batch_pf(k.pc_proof),
    };
    end_timer!(pf_timer);
    r
}

type LocalMarlinKZG10 = MarlinKZG10::<ark_bls12_377::Bls12_377, DensePolynomial<ark_bls12_377::Fr>>;
...
fn vk_publicize(vk: IndexVerifierKey<MpcVal<ark_bls12_377::Fr>, MpcMarlinKZG10>) -> IndexVerifierKey<ark_bls12_377::Fr, LocalMarlinKZG10> {
    IndexVerifierKey {
        index_info: index_info_publicize(vk.index_info),
        index_comms: vk.index_comms,
        verifier_key: vk.verifier_key,
    }
}
...
pub fn mpc_test_prove_and_verify(n_iters: usize) {
    ...
    let srs = MpcBlsMarlin::universal_setup(100, 50, 100, rng).unwrap();

    for _ in 0..n_iters {
        let a = MpcVal::<ark_bls12_377::Fr>::rand(rng);
        let b = MpcVal::<ark_bls12_377::Fr>::rand(rng);
        let empty_circuit: MySillyCircuit<MpcVal<ark_bls12_377::Fr>> = MySillyCircuit { a: None, b: None };
        let circ = MySillyCircuit {
            a: Some(a),
            b: Some(b),
        };
        ...
        let (index_pk, mpc_index_vk) = MpcBlsMarlin::index(&srs, empty_circuit.clone()).unwrap();
        let index_vk = vk_publicize(mpc_index_vk);
        let mpc_proof = MpcBlsMarlin::prove(&index_pk, circ, rng).unwrap();
        let proof = pf_publicize(mpc_proof);
        let public_a = a.publicize_unwrap();
        let is_valid = LocalBlsMarlin::verify(&index_vk, &inputs, &proof, rng).unwrap();
        assert!(is_valid);
        let is_valid = LocalBlsMarlin::verify(&index_vk, &[public_a], &proof, rng).unwrap();
        assert!(!is_valid);
    }
}
