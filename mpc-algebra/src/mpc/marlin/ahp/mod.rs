use crate::mpc::marlin::{String, ToString, Vec};
...
impl<F: PrimeField + crate::mpc::MpcWire> AHPForR1CS<F> {
    ...
    /// List of labels for polynomials in this proof
        ...
        E: EvaluationsProvider<F>,
    {
        ...
        let eval = evals.get_lc_eval(&outer_sumcheck, beta)?;
        dbg!(eval);
        dbg!(eval.publicize());
        //debug_assert!(eval.is_zero());
