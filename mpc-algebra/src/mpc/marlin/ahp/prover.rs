...
use crate::mpc::marlin::ahp::indexer::*;
use crate::mpc::marlin::ahp::verifier::*;
use crate::mpc::marlin::ahp::*;

use crate::mpc::marlin::ahp::constraint_systems::{
    make_matrices_square_for_prover, pad_input_for_indexer_and_prover, unformat_public_input,
};
use crate::mpc::marlin::{ToString, Vec};
use crate::MpcWire;
...
use ark_std::{
    cfg_into_iter, cfg_iter, cfg_iter_mut, start_timer, end_timer,
    io::{Read, Write},
};
...
fn divide_by_vanishing_poly_public_domain<F: PrimeField + MpcWire>(p: DensePolynomial<F>, domain: GeneralEvaluationDomain<F>) -> (DensePolynomial<F>, DensePolynomial<F>) {
    let shared = p.coeffs()[0].is_shared();
    assert!(p.coeffs().iter().all(|c| c.is_shared() == shared), "Non-homogeneous sharing");
    if shared {
        let p = DensePolynomial::from_coefficients_vec(p.coeffs().into_iter().map(|c| c.cast_to_public()).collect());
        let (q, r) = p.divide_by_vanishing_poly(domain).unwrap();
        let q = DensePolynomial::from_coefficients_vec(q.coeffs().into_iter().map(|c| c.cast_to_public()).collect());
        let r = DensePolynomial::from_coefficients_vec(r.coeffs().into_iter().map(|c| c.cast_to_public()).collect());
        (q, r)
    } else {
        p.divide_by_vanishing_poly(domain).unwrap()
    }
}

impl<F: PrimeField + MpcWire> AHPForR1CS<F> {
    ...
    pub fn prover_first_round<'a, R: RngCore>(
        ...
        dbg!(domain_x.size());
        dbg!(&w_poly);
        let (w_poly, _) = divide_by_vanishing_poly_public_domain(w_poly, domain_x);
        ...
        let scaled_sigma_1 = divide_by_vanishing_poly_public_domain(mask_poly.clone(), domain_h).1[0];
    ...
    pub fn prover_second_round<'a, R: RngCore>(
        ...
        let (h_1, x_g_1) = divide_by_vanishing_poly_public_domain(q_1, domain_h);
    ...
    pub fn prover_third_round<'a, R: RngCore>(
        ...
        let h_2 = divide_by_vanishing_poly_public_domain(&a_poly - &(&b_poly * &f), domain_k).0;
