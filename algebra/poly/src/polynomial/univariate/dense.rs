...
use crate::{
    univariate::DenseOrSparsePolynomial, EvaluationDomain, Evaluations, GeneralEvaluationDomain,
...
impl<F: FftField> fmt::Display for DensePolynomial<F> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_list()
            .entries(self.coeffs.iter().map(|f| format!("{}", f)))
            .finish()
    }
}
...
impl<'a, 'b, F: Field> Add<&'a F> for &'b DensePolynomial<F> {
    type Output = DensePolynomial<F>;

    fn add(self, other: &'a F) -> DensePolynomial<F> {
        let mut result = if self.is_zero() {
            self + &DensePolynomial::from_coefficients_vec(vec![*other])
        } else {
            let mut self_ = self.clone();
            self_.coeffs[0] += other;
            self_
        };
        result.truncate_leading_zeros();
        result
...
impl<'a, 'b, F: Field> AddAssign<(F, &'a DensePolynomial<F>)> for DensePolynomial<F> {
    fn add_assign(&mut self, (f, other): (F, &'a DensePolynomial<F>)) {
        ...
        if other.is_shared() && f.is_shared() {
            let mut fs = vec![f; other.len()];
            F::batch_product_in_place(&mut fs[..], &other.coeffs[..]);
            self.coeffs
                .iter_mut()
                .zip(fs)
                .for_each(|(a, b)| {
...
/// Performs O(nlogn) multiplication of polynomials if F is smooth.
impl<'a, 'b, F: FftField> Mul<&'a F> for &'b DensePolynomial<F> {
    type Output = DensePolynomial<F>;

    #[inline]
    fn mul(self, other: &'a F) -> DensePolynomial<F> {
        let mut self_ = self.clone();
        ark_std::cfg_iter_mut!(self_.coeffs).for_each(|c| *c *= other);
        self_
    }
}
    ...
    fn mul_by_vanishing_poly() {
        ...
        for size in 1..10 {
            ...
            for degree in 0..70 {
                ...
                let d_vanish: DensePolynomial<Fr> = domain.vanishing_polynomial().into();
                let ans2 = &p * &d_vanish;
                assert_eq!(ans1, ans2);
            }
        }
