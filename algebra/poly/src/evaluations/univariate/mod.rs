...
impl<F: FftField, D: EvaluationDomain<F> + fmt::Debug> fmt::Display for Evaluations<F, D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_list()
            .entries(self.evals.iter().map(|f| format!("{}", f)))
            .finish()
    }
}
...
impl<'a, 'b, F: FftField, D: EvaluationDomain<F>> Mul<&'a F>
    for &'b Evaluations<F, D>
{
    type Output = Evaluations<F, D>;

    #[inline]
    fn mul(self, other: &'a F) -> Evaluations<F, D> {
        let mut result = self.clone();
        result *= other;
        result
    }
...
impl<'a, F: FftField, D: EvaluationDomain<F>> MulAssign<&'a F>
    for Evaluations<F, D>
{
    #[inline]
    fn mul_assign(&mut self, other: &'a F) {
        let o = vec![*other; self.evals.len()];
        <F as Field>::batch_product_in_place(&mut self.evals, &o);
    }
}
...
impl<'a, 'b, F: FftField, D: EvaluationDomain<F>> Add<&'a F>
    for &'b Evaluations<F, D>
{
    type Output = Evaluations<F, D>;

    #[inline]
    fn add(self, other: &'a F) -> Evaluations<F, D> {
        let mut result = self.clone();
        result += other;
        result
    }
...
impl<'a, F: FftField, D: EvaluationDomain<F>> AddAssign<&'a F>
    for Evaluations<F, D>
{
    #[inline]
    fn add_assign(&mut self, other: &'a F) {
        ark_std::cfg_iter_mut!(self.evals)
            .for_each(|a| *a += other);
    }
}
